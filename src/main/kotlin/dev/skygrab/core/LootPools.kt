...
/**
 * Real loot pools per source, loaded once from the four JSON files under `/skygrab/pools/`
...
object LootPools {
    ...
    private fun resourceJson(name: String): JsonObject {
        val stream = LootPools::class.java.getResourceAsStream("/skygrab/pools/$name")
            ?: error("missing bundled resource /skygrab/pools/$name")
    ...
    private fun warnDropped(key: String, dropped: List<String>) {
        ...
        SkyGrab.LOGGER.warn("LootPools: $key dropped ${dropped.size} unresolved id(s): $dropped")
    }
