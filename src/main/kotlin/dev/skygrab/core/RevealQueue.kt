...
object RevealQueue {
    ...
    fun submit(r: Reveal) {
        if (!SkyGrabConfig.data.enabled) { ChatGuard.emitNow(r.heldChat); return }
    ...
    private fun pump() {
        ...
        mc.setScreenAndShow(CaseScreen(next, SkyGrabConfig.data.durationMs) { playing = false; pending = true })
    }
}
